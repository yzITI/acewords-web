<script>
  ...
  async function next () {
    ...
    if (currentPro && currentPro.due <= model.time()) return current = await model.lib.get(id2_id(currentPro.id))
    // new word
    ...
    // find review loosely
    if (currentPro && currentPro.due <= model.time() + 25) return current = await model.lib.get(id2_id(currentPro.id))
    complete()
  }
...
<div class="h-screen w-screen px-4 py-16 bg-gray-100 flex flex-col items-center justify-between">
  {#if current.word}
    <div class="flex flex-col items-center w-full" style="max-width: 360px;">
      <h2 class="text-4xl font-bold">{current.word}</h2>
      <p class="italic text-gray-700 my-2">/{current.phonetic}/</p>
      <div class={'w-full border bg-white rounded p-2 my-6 transition-all ' + (show ? 'opacity-100' : 'opacity-0')}>
    ...
    <div class="flex items-center justify-between my-4 font-bold text-lg w-full" style="max-width: 400px;">
      {#if show}
        <button on:click={() => response(false)} class="transition-all shadow hover:shadow-md rounded w-1/2 p-2 bg-red-500 text-white mx-2">不认识</button>
        <button on:click={() => response(true)} class="transition-all shadow hover:shadow-md rounded w-1/2 p-2 bg-green-500 text-white mx-2">认识</button>
      {:else}
