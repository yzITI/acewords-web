<script>
  ...
  import { loading, groups } from '$lib/stores.js'
...
<div class="min-h-screen w-full px-4 sm:px-10 py-10 bg-gray-100">
  ...
  <div class="mt-6 text-lg font-bold text-gray-700 select-none">
    {#each $groups as g}
      <div class="rounded p-3 my-2 transition-all shadow hover:shadow-md cursor-pointer flex items-center bg-white" on:keypress={() => goto('/group/' + g._id)} on:click={() => goto('/group/' + g._id)}>
        <AIcon path={mdiAccountGroupOutline} size="1.5rem" color="rgb(55 65 81)" />
        <span class="ml-2">{g.name}</span>
      </div>
    {/each}
    {#if !$groups?.length && !fetching}
